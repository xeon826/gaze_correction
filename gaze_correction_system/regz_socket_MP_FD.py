...
class video_receiver:
    def __init__(self, shared_v, lock):
        ...
        self.predictor = dlib.shape_predictor(
            current_dir + "/lm_feat/shape_predictor_68_face_landmarks.dat"
        )
...
class gaze_redirection_system:
        ...
        try:
            ...
            for line in geometry.splitlines():
                if "Position" in line:
                    ...
                    left, top = map(int, position[0].split(","))
                if "Geometry" in line:
    ...
    def shifting_angles_estimator(self, R_le, R_re, shared_v, lock):
        ...
        Pw = (
            self.Ps[0] * (R_w[0] - Rs[0] / 2) / Rs[0],
            self.Ps[1] * (R_w[1] - Rs[1] / 2) / Rs[1],
            0,
        )
